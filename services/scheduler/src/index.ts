...
async function parseViewsFromUrl(
  url: string,
): Promise<{ viewsToday?: number; status?: boolean }> {
  console.log(`🔍 Parsing views and status from: ${url}`)

  try {
    // Используем Python API парсер
    const response = await fetch(
      `http://localhost:8008/api/parse/single?url=${encodeURIComponent(url)}`,
      {
        method: 'GET',
        headers: {
    ...
    if (
      ...
      propertyData.is_active !== null
    ) {
      status = Boolean(propertyData.is_active)
    } else if (
      propertyData.isActive !== undefined &&
      propertyData.isActive !== null
    ) {
      status = Boolean(propertyData.isActive)
    } else if (
      propertyData.status !== undefined &&
...
async function sendDailyUpdate(
  ...
  try {
    const response = await fetch(
      'http://localhost:13001/scheduler/daily-update',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.log(
      `📊 Ad ${adId}: daily update sent successfully, changes: ${result.changes}`,
    )
    return result
  } catch (error) {
...
async function performDailyTracking() {
  ...
  try {
    ...
    const adsToTrack = allAds.filter((ad) => {
      ...
      return (
        source === 'cian' || // Все CIAN
        (ad.sma === 1 && (source === 'avito' || source === 'yandex'))
      ) // Только в сравнении для Avito/Yandex
    })

    console.log(
      `📋 Found ${adsToTrack.length} ads to track out of ${allAds.length} total`,
    )
    ...
    try {
      ...
      for (let i = 0; i < processedIds.length; i += batchSize) {
        ...
        try {
          const response = await fetch(
            'http://localhost:13001/ads/transfer-to-public',
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
          ...
          if (response.ok) {
            const result = await response.json()
            console.log(
              `✅ Successfully synced batch ${i / batchSize + 1}: ${result.message}`,
            )
          } else {
            const error = await response.text()
            console.warn(
              `⚠️ Failed to sync batch ${i / batchSize + 1}: ${error}`,
            )
          }
        } catch (syncError) {
          console.error(
            `❌ Error syncing batch ${i / batchSize + 1}:`,
            syncError,
          )
        }
