...
export default function EditFlatForm({
  ...
  const { data: broaderAdsFromFindAds = [] } = useBroaderAdsFromFindAds(
    flat?.id || 0,
  )
  const {
    data: nearbyAdsFromFindAdsResponse = { ads: [], filters: null, count: 0 },
    refetch: refetchNearbyAds,
    isLoading: isLoadingNearbyAds,
  } = useNearbyAdsFromFindAds(flat?.id || 0)
  const nearbyAdsFromFindAds = nearbyAdsFromFindAdsResponse?.ads || []
  ...
  const groupedBroaderAds = useMemo(() => {
    ...
    groups.forEach((groupAds) => {
      ...
      if (cianAds.length > 0) {
        ...
        const latestCianAd = cianAds.sort(
          (a, b) =>
  ...
  const exportComparisonToExcel = () => {
    ...
    const exportData = comparisonAds.map((ad) => ({
      ...
      Статус: ad.status || '',
      'Просмотры на дату':
        ad.viewsToday !== null && ad.viewsToday !== undefined
          ? ad.viewsToday
          : '—',
    }))
  ...
  const prepareUpdateData = (parsedData: any) => {
    ...
    if (typeof parsedData.price === 'number' && parsedData.price > 0) {
      ...
      typeof parsedData.price === 'string' &&
      parsedData.price !== 'Не найдено'
    ) {
    ...
    if (
      typeof parsedData.total_floors === 'number' ||
      typeof parsedData.totalFloors === 'number'
    ) {
    ...
    if (parsedData.views_today || parsedData.viewsToday) {
      ...
      updateData.viewsToday =
        typeof views === 'number' ? views : parseInt(String(views))
    }
  ...
  const { mutateAsync: parseProperty, isPending: isParsing } =
    useParseProperty()
      ...
      for (const broaderAd of broaderAds) {
        try {
          ...
          if (existingUrls.has(broaderAd.url)) {
            console.log(
              `Объявление с URL ${broaderAd.url} уже существует, пропускаем`,
            )
            skippedCount++
            continue
          }
  ...
  const autoFindSimilarAds = useCallback(async () => {
    ...
    try {
      ...
      const similar: SimilarAd[] =
        ads.length > 0 && ads[0]?.id
      ...
      for (const similarAd of similar) {
        try {
          ...
          if (existingUrls.has(similarAd.url)) {
            console.log(
              `Объявление с URL ${similarAd.url} уже существует, пропускаем`,
            )
            skippedCount++
            continue
          }
  ...
  const handleUpdateHouseAds = async () => {
    ...
    console.log(
      `=== Начинаем обновление ${groupedBroaderAds.length} объявлений по дому ===`,
    )

    // Определяем объявления по источникам
    const cianAds = groupedBroaderAds.filter((ad) => ad.url.includes('cian.ru'))
    const avitoAds = groupedBroaderAds.filter((ad) =>
      ad.url.includes('avito.ru'),
    )
    const yandexAds = groupedBroaderAds.filter((ad) =>
      ad.url.includes('yandex.ru'),
    )

    console.log(
      `Источники: Cian (${cianAds.length}), Avito (${avitoAds.length}), Yandex (${yandexAds.length})`,
    )
        ...
        for (const houseAd of cianAds) {
          ...
          if (result.success && result.data) {
            ...
            if (adsItem) {
              ...
              totalUpdated++
              console.log(
                `✅ Cian объявление по дому ${adsItem.id} обновлено (цена: ${updateData.price || 'не изменена'}, статус: ${updateData.status})`,
              )
            } else {
              console.log(
                `⚠️ Не найден соответствующий объект в ads для URL: ${houseAd.url}`,
              )
            }
          } else {
            totalErrors++
            console.log(
              `❌ Не удалось спарсить Cian объявление по дому: ${houseAd.url}`,
            )
          }
        }
        toast.success(
          `Cian (дом): обновлено ${totalUpdated - totalErrors} из ${cianAds.length} объявлений`,
        )
      } catch (error) {
        ...
        for (const houseAd of avitoAds) {
          ...
          if (result.success && result.data) {
            ...
            if (adsItem) {
              ...
              totalUpdated++
              console.log(
                `✅ Avito объявление по дому ${adsItem.id} обновлено (цена: ${updateData.price || 'не изменена'}, статус: ${updateData.status})`,
              )
            } else {
              console.log(
                `⚠️ Не найден соответствующий объект в ads для URL: ${houseAd.url}`,
            ...
            totalErrors++
            console.log(
              `❌ Не удалось спарсить Avito объявление по дому: ${houseAd.url}`,
            )
          }
        }
        toast.success(
          `Avito (дом): обновлено ${totalUpdated - totalErrors} из ${avitoAds.length} объявлений`,
        )
      } catch (error) {
    ...
    if (yandexAds.length > 0) {
      ...
      console.log(
        `⚠️ Yandex объявления по дому (${yandexAds.length}) - функционал не реализован`,
      )
      setTimeout(() => {
        setIsUpdatingHouseYandex(false)
        toast.info(
          `Yandex (дом, ${yandexAds.length} объявлений): функционал пока не реализован`,
        )
      }, 1000)
    }
    ...
    // Итоговый результат
    console.log(
      `=== Обновление по дому завершено: ${totalUpdated} успешно, ${totalErrors} ошибок ===`,
    )
  ...
  const handleUpdateComparisonAds = async () => {
    ...
    console.log(
      `=== Начинаем обновление ${comparisonAds.length} объявлений в сравнении ===`,
    )
    ...
    if (cianAds.length > 0) {
      ...
      console.log(
        `📊 Обновляем ${cianAds.length} объявлений Cian в сравнении...`,
      )
      ...
      try {
        for (const ad of cianAds) {
          ...
          if (result.success && result.data) {
            ...
            totalUpdated++
            console.log(
              `✅ Cian объявление в сравнении ${ad.id} обновлено (цена: ${updateData.price || 'не изменена'}, статус: ${updateData.status})`,
            )
            ...
            totalErrors++
            console.log(
              `❌ Не удалось спарсить Cian объявление в сравнении ${ad.id}`,
            )
          }
        }
        toast.success(
          `Cian (сравнение): обновлено ${totalUpdated - totalErrors} из ${cianAds.length} объявлений`,
        )
      } catch (error) {
    ...
    if (avitoAds.length > 0) {
      ...
      console.log(
        `📊 Обновляем ${avitoAds.length} объявлений Avito в сравнении...`,
      )
      ...
      try {
        for (const ad of avitoAds) {
          ...
          if (result.success && result.data) {
            ...
            totalUpdated++
            console.log(
              `✅ Avito объявление в сравнении ${ad.id} обновлено (цена: ${updateData.price || 'не изменена'}, статус: ${updateData.status})`,
            )
            ...
            totalErrors++
            console.log(
              `❌ Не удалось спарсить Avito объявление в сравнении ${ad.id}`,
            )
          }
        }
        toast.success(
          `Avito (сравнение): обновлено ${totalUpdated - totalErrors} из ${avitoAds.length} объявлений`,
        )
      } catch (error) {
    ...
    if (yandexAds.length > 0) {
      ...
      console.log(
        `⚠️ Yandex объявления в сравнении (${yandexAds.length}) - функционал не реализован`,
      )
      setTimeout(() => {
        setIsUpdatingComparisonYandex(false)
        toast.info(
          `Yandex (сравнение, ${yandexAds.length} объявлений): функционал пока не реализован`,
        )
      }, 1000)
    }
    ...
    // Итоговый результат
    console.log(
      `=== Обновление сравнения завершено: ${totalUpdated} успешно, ${totalErrors} ошибок ===`,
    )
  ...
  const handleUpdateAllSources = async () => {
    ...
    console.log(
      `=== Начинаем обновление ${flatAdsToUpdate.length} объявлений ===`,
    )
    ...
    const yandexAds = flatAdsToUpdate.filter((ad) =>
      ad.url.includes('yandex.ru'),
    )
      ...
      try {
        for (const ad of cianAds) {
          ...
          if (result.success && result.data) {
            ...
            totalUpdated++
            console.log(
              `✅ Cian объявление ${ad.id} обновлено (цена: ${updateData.price || 'не изменена'}, статус: ${updateData.status})`,
            )
        ...
        toast.success(
          `Cian: обновлено ${totalUpdated - totalErrors} из ${cianAds.length} объявлений`,
        )
      } catch (error) {
      ...
      try {
        for (const ad of avitoAds) {
          ...
          if (result.success && result.data) {
            ...
            totalUpdated++
            console.log(
              `✅ Avito объявление ${ad.id} обновлено (цена: ${updateData.price || 'не изменена'}, статус: ${updateData.status})`,
            )
        ...
        toast.success(
          `Avito: обновлено ${totalUpdated - totalErrors} из ${avitoAds.length} объявлений`,
        )
      } catch (error) {
    ...
    if (yandexAds.length > 0) {
      ...
      console.log(
        `⚠️  Yandex объявления (${yandexAds.length}) - функционал не реализован`,
      )
      setTimeout(() => {
        setIsUpdatingFlatYandex(false)
        toast.info(
          `Yandex (${yandexAds.length} объявлений): функционал пока не реализован`,
        )
      }, 1000)
    }
    ...
    // Итоговый результат
    console.log(
      `=== Обновление завершено: ${totalUpdated} успешно, ${totalErrors} ошибок ===`,
    )
          ...
          <Page>
                ...
                <Form.Field
                  ...
                  render={({ field }) => (
                    <Form.Item className='space-y-1 flex-1'>
                      <Form.Label className='text-sm text-muted-foreground'>
                        Адрес
                      </Form.Label>
                      <Form.Control>
                        <Input
            ...
            <Page.Content className='divide-y *:py-5 first:*:pt-0 last:*:pb-0'>
                      ...
                      {(isUpdatingFlatCian ||
                        ...
                        <div className='flex items-center gap-3 text-sm'>
                          <div className='flex items-center gap-1'>
                            ...
                            {isUpdatingFlatCian ? (
                              <svg
                                className='w-3 h-3 animate-spin text-orange-600'
                                fill='none'
                                viewBox='0 0 24 24'
                              >
                                <circle
                      ...
                      <thead className='[&_tr]:border-b'>
                        <tr className='border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted'>
                          ...
                          <th className='h-12 px-4 text-left align-middle font-medium text-muted-foreground [&:has([role=checkbox])]:pr-0'>
                            <div className='flex items-center gap-2'>Цена</div>
                          </th>
                          <th className='h-12 px-4 text-left align-middle font-medium text-muted-foreground [&:has([role=checkbox])]:pr-0'>
                            <div className='flex items-center gap-2'>
                              Создано
                          ...
                          <th className='h-12 px-4 text-left align-middle font-medium text-muted-foreground [&:has([role=checkbox])]:pr-0'>
                            <div className='flex items-center gap-2'>Автор</div>
                          </th>
                      ...
                      <tbody className='[&_tr:last-child]:border-0'>
                        {flatAdsFromFindAds.length === 0 ? (
                          <tr className='border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted'>
                            ...
                            >
                              <div className='text-sm text-center'>
                                Нет объявлений по этой квартире
                              </div>
                            </td>
                          </tr>
                        ) : (
                          flatAdsFromFindAds.map((findAdsItem) => {
                            ...
                            const adsItem = ads.find(
                              (ad) => ad.url === findAdsItem.url,
                            )
                            const loadButton = adsItem && (
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    console.log(
                                      'Начинаем парсинг объявления:',
                                      findAdsItem.url,
                                    )
                                    const result = await parseProperty(
                                      findAdsItem.url,
                                    )
                                    ...
                                    if (result.success && result.data) {
                                      ...
                                      toast.success(
                                        'Данные объявления обновлены успешно!',
                                      )
                                      console.log(
                                        'Объявление успешно обновлено в БД',
                                      )
                                    } else {
                                      console.error(
                                        'API вернул ошибку:',
                                      ...
                                      toast.error(
                                        `Ошибка парсинга: ${result.message}`,
                                      )
                                    }
                                  } catch (error) {
                                    console.error(
                                    ...
                                    toast.error(
                                      'Ошибка при загрузке данных объявления',
                                    )
                                  }
                                }}
                              >
                                  ...
                                  if (
                                    ...
                                    try {
                                      ...
                                      try {
                                        ...
                                        console.warn(
                                          'Ошибка при refetch после удаления:',
                                          err,
                                        )
                                      }
                                    } catch (error) {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className='p-4 align-middle text-sm'>
                                  {findAdsItem.price > 0
                                    ? `${findAdsItem.price.toLocaleString()} ₽`
                                    : '-'}
                                </td>
                                {/* Создано */}
                                <td className='p-4 align-middle text-sm'>
                                ...
                                <td className='p-4 align-middle text-sm text-center'>
                                  {(() => {
                                    ...
                                    const isInComparison = ads.some(
                                      (ad) =>
                                        ad.url === findAdsItem.url &&
                                        ad.sma === 1,
                                    )
                                    return (
                                      <button
                                        type='button'
                                        onClick={() =>
                                          handleAddToComparison(findAdsItem)
                                        }
                          ...
                          groupedBroaderAds.map((findAdsItem) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className='p-2 align-middle text-sm'>
                                  {formatPriceInMillions(findAdsItem.price)}
                                </td>
                                <td className='p-2 align-middle text-sm'>
                                  {findAdsItem.rooms}
                                </td>
                                ...
                                <td className='p-2 align-middle text-sm'>
                                  {findAdsItem.created
                                    ? formatDate(findAdsItem.created)
                                    : '-'}
                                </td>
                                <td className='p-2 align-middle text-sm'>
                                  {adsItem?.updatedAt
                                    ? formatDate(adsItem.updatedAt)
                                    : findAdsItem.updated
                                      ? formatDate(findAdsItem.updated)
                                      : '-'}
                                </td>
                                <td className='p-2 align-middle text-sm'>
                                  {findAdsItem.person_type || '-'}
              ...
              <div className='py-4 px-4 bg-gray-50 rounded-lg mb-4'>
                <div className='flex items-center justify-between mb-4'>
                  <h3 className='text-lg font-medium'>
                    Объявления в радиусе 500м и дешевле
                  </h3>
                  <button
                    type='button'
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.success(
                          'Данные о близлежащих объявлениях обновлены',
                        )
                      } catch (error) {
                        console.error(
                          'Ошибка при обновлении близлежащих объявлений:',
                          error,
                        )
                      ...
                      <tbody className='[&_tr:last-child]:border-0'>
                        {nearbyAdsFromFindAds.length === 0 ? (
                          <tr className='border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted'>
                            ...
                            >
                              <div className='text-sm text-center'>
                                Нет близлежащих объявлений
                              </div>
                            </td>
                          </tr>
                        ) : (
                          nearbyAdsFromFindAds.map(
                            (findAdsItem: any, index: number) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className='p-2 align-middle text-sm'>
                                    {(findAdsItem.price / 1000000).toFixed(2)}
                                  </td>
                                  <td className='p-2 align-middle text-sm'>
                                    {findAdsItem.rooms}
                                  </td>
                            ...
                            >
                              {mounted && expandedView ? (
                                <>
                                  ...
                                  <td className='p-2 align-middle text-sm'>
                                    {formatArea(ad.totalArea)}
                                  </td>
                                  <td className='p-2 align-middle text-sm'>
                                    {formatArea(ad.livingArea)}
                                  </td>
                                  <td className='p-2 align-middle text-sm'>
                                    {formatArea(ad.kitchenArea)}
                                  </td>
                                  <td className='p-2 align-middle text-sm'>
                                    {ad.floor || ''}
                                  </td>
